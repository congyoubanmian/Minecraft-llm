...
第十八批·全城精装（总量 124,911 → 126,063 fills，+1,152）：

- `lantern_wheel_3d.py`（415 fills）：西市北口安福门灯轮——竖直
  scanline 真圆灯轮（GOLD 外圈+RED 内圈双圈轮辋、8 轮辐、五色
  灯组 20 盏按 18° 均布）、中心灯柱高 y37、圆台基座、同心放射
  地灯阵 52 盏、彩楼看棚两座。
- `official_interior_3d.py`（315 fills）：四官署堂内——公案+官员
  坐像+跪垫（差异陈设：尚书省兵器架舆图壁画、御史台獬豸像弹
  章匣、大理寺卷宗架囚栏、鸿胪寺贡品地毯），文书房、登闻鼓、
  影壁。贴附率 53.5%（家具堆叠正常稀释）。
- `city_roof_beasts_3d.py`（422 fills）：全城脊兽扩展——复用
  roof_color_zoning 坐标体系，寺（青底金兽 39）/署（黄底 30）/
  市（灰底 24）/宅（灰白 24）四类 47 条正脊 117 只走兽+188 枚
  翼角套兽，全部锚定脊帽（悬空 0），与 palace_roof_detail 最近
  距离 409 格。

执行备份：`backups/world_backup_20260905_173557`。
