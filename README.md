...
首页项目列表现在不仅用于打开历史项目，也承担轻量维护工作：

- 支持按项目名、ID、状态、描述、模板猜测和建筑类型搜索
- 支持筛选已完成、生成中、失败、已取消、有蓝图、有 Schematic、缺失快照项目
- 支持按最近更新、名称、方块数、体积、快照占用、缺失快照数量排序
- 会记住搜索、筛选和排序偏好，并提供一键重置
- 项目卡会显示快照可用数量、模块数量、占用空间、最近快照时间和缺失快照数量
- 列表顶部会汇总当前可见项目的快照占用，以及缺失快照项目数和记录数
- 如果快照记录指向的 `.schem` 文件已经不存在，可以在项目卡上清理单个项目，也可以清理当前可见列表中的全部缺失快照记录

快照用于模块替换、回滚和世界区域恢复。清理“缺失快照”只删除已经无法使用的记录，不会删除仍然存在的 `.schem` 文件，也不会修改 Minecraft 世界。
...
POST /api/placements/{project_id}/teleport
POST /api/placements/{project_id}/archive
POST /api/placements/{project_id}/clear
```
...
区域操作支持传送、归档和清空。清空会执行 `fill ... air replace`，默认限制 500 万方块以内，并要求：
